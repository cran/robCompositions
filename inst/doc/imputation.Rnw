...
\vspace{12pt}
\noindent {\bf Keywords:} Missing Values, Compositional Data, Imputation.
...
v1 <- variation(acomp(x$z2[1:95,]))
v2 <- variation(acomp(xImp1$xImp[1:95,]))
v22 <- variation(acomp(xImp2$xImp[1:95,]))
...
bootimp <- function(x, R = 100, method = "lm") {
     ...
     invisible(as.data.frame(acomp(med)))
...
%@
\includegraphics{imputation-boxplot.pdf}
\vspace*{-1cm}
